...
export function allowedDisplayCidrs() {
...
export function requestMatchesDisplayCidrs(request: Request) {
  const address = clientIp(request);
  if (!address) {
    return false;
  }
  return allowedDisplayCidrs().some((cidr) => ipInCidr(address, cidr));
}

export function clientIp(request: Request): string | undefined {
  const forwarded = request.header("x-forwarded-for")?.split(",")[0]?.trim();
  const candidate = forwarded || request.ip || request.socket.remoteAddress || undefined;
  if (!candidate) {
    return undefined;
  }
...
function ipv4InCidr(address: string, range: string, prefix: number) {
  ...
  if (addressValue === undefined || rangeValue === undefined) {
    return false;
  }
...
function expandIpv6(value: string) {
  ...
  const hasIpv4Tail = tailParts[tailParts.length - 1]?.includes(".") || headParts[headParts.length - 1]?.includes(".");
  const mappedHead = expandEmbeddedIpv4(headParts);
  const mappedTail = expandEmbeddedIpv4(tailParts);
  if (!mappedHead || !mappedTail) {
    return undefined;
  }

  const missing = 8 - (mappedHead.length + mappedTail.length);
  if ((head && tail && missing < 1) || (!value.includes("::") && missing !== 0) || hasIpv4Tail && mappedHead.length + mappedTail.length !== 8) {
    return undefined;
  }

  return [...mappedHead, ...Array(Math.max(missing, 0)).fill(0), ...mappedTail];
}
